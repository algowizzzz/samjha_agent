...
from external.products.model_doc.models import AgentConfig, AgentState
...
def summarize_file(file_content: str) -> str:
    ...
    return client.invoke_with_prompt("You are a code documentation assistant.", prompt)


def generate_hierarchical_summary(summaries: list) -> str:
    """Generate hierarchical summary from file summaries."""
    if not is_llm_available():
        raise LLMNotAvailableError("LLM not configured")
    client = get_llm_client()
    import json
    prompt = f"Create a hierarchical summary from these file summaries:\n\n{json.dumps(summaries, indent=2)}"
    return client.invoke_with_prompt("You are a code documentation assistant.", prompt)


def create_documentation_outline(hierarchy: str) -> dict:
    """Create documentation outline from hierarchy."""
    if not is_llm_available():
        raise LLMNotAvailableError("LLM not configured")
    client = get_llm_client()
    prompt = f"Create a documentation outline from this hierarchy:\n\n{hierarchy}"
    response = client.invoke_with_prompt("You are a code documentation assistant.", prompt, response_format="json")
    import json
    try:
...
def draft_section(section_info: dict, context: str) -> str:
    """Draft a documentation section."""
    if not is_llm_available():
        raise LLMNotAvailableError("LLM not configured")
    client = get_llm_client()
    import json
    prompt = f"Draft documentation for this section:\n\nSection: {json.dumps(section_info)}\n\nContext: {context}"
    return client.invoke_with_prompt("You are a code documentation assistant.", prompt)
