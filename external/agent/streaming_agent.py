...
from external.agent.schemas import AgentState
...
class StreamingAgent:
    ...
    def run_query_streaming(
        ...
        data_dict_file: Optional[str] = None,
        agent_prompts_file: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            from external.agent.graph_nodes import (
            ...
            agent_prompts_path = None
            if agent_prompts_file:
                # Build full path based on file type
                import os
                if os.path.exists(os.path.join("external", "config", "agent", agent_prompts_file)):
                    agent_prompts_path = os.path.join("external", "config", "agent", agent_prompts_file)
                elif os.path.exists(agent_prompts_file):
                    agent_prompts_path = agent_prompts_file
            
            cfg = QueryAgentConfig(config_path=agent_prompts_path) if agent_prompts_path else QueryAgentConfig()
            state_manager = AgentStateManager()
            ...
            state: AgentState = {
                ...
                # Store config file selections in state for nodes to use
                "config_files": {
            ...
            while steps < max_steps:
                ...
                if self.session_manager.is_cancelled(sid):
                    logger.info(f"[StreamingAgent] Query cancelled for session {sid}")
                    state["query_cancelled"] = True
                    state["control"] = "end"
            ...
            # Check if query was cancelled before finalizing
            query_cancelled = state.get("query_cancelled", False) or self.session_manager.is_cancelled(sid)
            
            # Always call end_node unless waiting for user clarification or cancelled
            if query_cancelled:
                # Query was cancelled - don't save to conversation history
                logger.info(f"[StreamingAgent] Query cancelled, skipping end_node and conversation history save")
                state["query_cancelled"] = True
                self._emit('agent:cancelled', {
                    'session_id': sid,
                    'message': 'Query cancelled by user'
                })
                # Clean up and return without saving
                self.session_manager.unregister_session(sid)
                return {
                    "query_cancelled": True,
                    "session_id": sid,
                    "message": "Query cancelled by user"
                }
            elif state.get("control") == "wait_for_user":
    ...
    def resume_with_clarification_streaming(
        ...
        try:
            ...
            # Update config files in state if provided (user may have changed dropdowns)
            if data_dict_file or agent_prompts_file:
                if "config_files" not in state:
                    state["config_files"] = {}
                if data_dict_file:
                    state["config_files"]["data_dict_file"] = data_dict_file
                if agent_prompts_file:
                    state["config_files"]["agent_prompts_file"] = agent_prompts_file
